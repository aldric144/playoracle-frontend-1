...
const MMACard = ({ event }: { event: MMAEvent }) => {
  const [isFlipped, setIsFlipped] = useState(false);

  return (
    <div 
      className="mma-card-container"
      style={{ perspective: '1000px' }}
    >
      <div 
        className={`mma-card ${isFlipped ? 'flipped' : ''}`}
      ...
      >
        ...
        >
          <Card className="h-full bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 border-gray-700 p-6 hover:border-red-500/50 transition-colors">
              ...
              <div className="flex-1 flex flex-col justify-center gap-4">
                <div className="text-center">
                  ...
                  <div className="flex items-center justify-center gap-2 mt-1">
                    {event.fighter_one.stance && (
                      <span className="text-xs text-gray-500">{event.fighter_one.stance}</span>
                    )}
                    {event.fighter_one.nationality && (
                      <span className="text-xs text-gray-500">{event.fighter_one.nationality}</span>
                    )}
                  </div>
                </div>
                ...
                <div className="text-center">
                  ...
                  <div className="flex items-center justify-center gap-2 mt-1">
                    {event.fighter_two.stance && (
                      <span className="text-xs text-gray-500">{event.fighter_two.stance}</span>
                    )}
                    {event.fighter_two.nationality && (
                      <span className="text-xs text-gray-500">{event.fighter_two.nationality}</span>
                    )}
                  </div>
                </div>
              </div>
        ...
        >
          <Card className="h-full bg-gradient-to-br from-red-900/50 via-gray-800 to-gray-900 border-red-500/30 p-6">
            <div className="flex flex-col h-full justify-between">
...
export const MMADashboard = () => {
  ...
  useEffect(() => {
    const fetchMMAEvents = async () => {
      try {
        setLoading(true);
        const response = await api.getMMAEvents();
        setEvents(response.events || []);
      } catch (err) {
        console.error('Failed to fetch MMA events:', err);
        setError('Failed to load MMA events');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (events.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-400">No upcoming MMA events available</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {events.map((event) => (
          <MMACard key={event.id} event={event} />
        ))}
      </div>

      <div className="text-center text-xs text-gray-500 mt-8">
        <p>Click any fight card to view DCI analysis</p>
        <p className="mt-1">Data powered by UFC & MMA databases • Updated daily</p>
      </div>
    </div>
  );
};
