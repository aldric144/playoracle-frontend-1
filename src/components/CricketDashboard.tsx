...
const CricketCard = ({ event }: { event: CricketEvent }) => {
  const [isFlipped, setIsFlipped] = useState(false);

  return (
    <div 
      className="cricket-card-container"
      style={{ perspective: '1000px' }}
    >
      <div 
        className={`cricket-card ${isFlipped ? 'flipped' : ''}`}
...
export const CricketDashboard = () => {
  ...
  useEffect(() => {
    const fetchCricketEvents = async () => {
      try {
        setLoading(true);
        const response = await api.getCricketEvents();
        setEvents(response.events || []);
      } catch (err) {
        console.error('Failed to fetch cricket events:', err);
        setError('Failed to load cricket events');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (events.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-400">No upcoming cricket events available</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {events.map((event) => (
          <CricketCard key={event.id} event={event} />
        ))}
      </div>

      <div className="text-center text-xs text-gray-500 mt-8">
        <p>Click any match card to view DCI analysis</p>
        <p className="mt-1">Data powered by ICC databases • Updated daily</p>
      </div>
    </div>
  );
};
