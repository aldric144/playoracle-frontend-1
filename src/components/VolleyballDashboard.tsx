...
const VolleyballCard = ({ event }: { event: VolleyballEvent }) => {
  const [isFlipped, setIsFlipped] = useState(false);

  return (
    <div 
      className="volleyball-card-container"
      style={{ perspective: '1000px' }}
    >
      <div 
        className={`volleyball-card ${isFlipped ? 'flipped' : ''}`}
      ...
      >
        <div 
          className="volleyball-card-front"
          style={{
            position: 'absolute',
            width: '100%',
            height: '100%',
        ...
        >
          <Card className="h-full bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 border-gray-700 p-6 hover:border-orange-500/50 transition-colors">
            <div className="flex flex-col h-full justify-between">
              ...
              <div className="flex-1 flex flex-col justify-center gap-4">
                ...
                <div className="text-center">
                  <span className="text-2xl font-bold text-orange-400">VS</span>
                </div>
        ...
        <div 
          className="volleyball-card-back"
          style={{
            position: 'absolute',
            width: '100%',
            height: '100%',
        ...
        >
          <Card className="h-full bg-gradient-to-br from-orange-900/50 via-gray-800 to-gray-900 border-orange-500/30 p-6">
            <div className="flex flex-col h-full justify-between">
...
export const VolleyballDashboard = () => {
  const [events, setEvents] = useState<VolleyballEvent[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const fetchVolleyballEvents = async () => {
      try {
        setLoading(true);
        const response = await api.getVolleyballEvents();
        setEvents(response.events || []);
      } catch (err) {
        console.error('Failed to fetch volleyball events:', err);
        setError('Failed to load volleyball events');
      } finally {
        setLoading(false);
      }
    };

    fetchVolleyballEvents();
  }, []);
  ...
  if (events.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-400">No upcoming volleyball events available</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {events.map((event) => (
          <VolleyballCard key={event.id} event={event} />
        ))}
      </div>

      <div className="text-center text-xs text-gray-500 mt-8">
        <p>Click any match card to view DCI analysis</p>
        <p className="mt-1">Data powered by FIVB databases • Updated daily</p>
      </div>
    </div>
  );
};
