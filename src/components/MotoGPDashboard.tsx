...
interface MotoGPDashboardProps {
  onBack: () => void;
}

export const MotoGPDashboard: React.FC<MotoGPDashboardProps> = ({ onBack }) => {
  ...
  const fetchMotoGPData = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/motogp/upcoming?scope=${scope}`);
      const racesData = await response.json();
      
      const historyResponse = await fetch(`${import.meta.env.VITE_API_URL}/api/motogp/history`);
      const historyData = await historyResponse.json();

      setRaces(racesData.races || []);
      setHistory(historyData.history || []);
    } catch (error) {
      console.error('Failed to fetch MotoGP data:', error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6 p-6 bg-[#0a0f1f] ">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-3xl font-bold text-zinc-100">MotoGP Races</h1>
        </div>
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <p className="text-sm text-zinc-400">{race.circuit}</p>
              ...
              <div className="space-y-2 mb-4">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-emerald-400">{race.class}</span>
                </div>
              </div>
      ...
      {races.length === 0 && !loading && (
        <div className="text-center py-12 bg-[#101626] border border-emerald-500/20 rounded-xl shadow-lg shadow-emerald-500/10">
          <Bike className="w-16 h-16 text-zinc-600 mx-auto mb-4" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {history.map((race, idx) => (
          ...
          >
            ...
            <p className="text-sm text-zinc-400 mb-4">{race.circuit}</p>
