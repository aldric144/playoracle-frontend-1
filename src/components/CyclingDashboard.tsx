...
export const CyclingDashboard: React.FC<CyclingDashboardProps> = ({ onBack }) => {
  ...
  const fetchCyclingData = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/cycling/upcoming?scope=${scope}`);
      const racesData = await response.json();
      
      const historyResponse = await fetch(`${import.meta.env.VITE_API_URL}/api/cycling/history`);
    ...
    <div className="space-y-6 p-6 bg-[#0a0f1f]">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-3xl font-bold text-zinc-100">Cycling Races</h1>
        </div>
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  {race.stage && <p className="text-sm text-zinc-400">{race.stage}</p>}
              ...
              <div className="space-y-2 mb-4">
                ...
                {race.route && (
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium text-xs">{race.route}</span>
                  </div>
                )}
                <div className="flex justify-between text-sm">
                ...
                {race.elevation_gain && (
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium text-emerald-400">{race.elevation_gain}</span>
                  </div>
                )}
                <div className="flex justify-between text-sm">
                  <span className="text-zinc-400">Category:</span>
                  <span className="font-medium text-emerald-400">{race.category}</span>
                </div>
              </div>

              {/* Top Riders */}
              {race.top_riders && race.top_riders.length > 0 && (
                <div className="border-t border-zinc-700 pt-4">
                  <h4 className="text-sm font-semibold text-zinc-300 mb-3">Top Contenders</h4>
                  <div className="space-y-3">
                    {race.top_riders.slice(0, 3).map((rider: any, ridx: number) => (
                      <div key={ridx} className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-bold text-zinc-500">#{rider.number}</span>
                          <div>
                            <div className="text-sm font-medium">{rider.name}</div>
                            <div className="text-xs text-zinc-500">{rider.team}</div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className={`px-2 py-1 rounded text-xs font-bold text-white ${getDCIColor(rider.dci.dci_score)}`}>
                            DCI {rider.dci.dci_score}
                          </span>
                        </div>
                      </div>
                    ))}
      ...
      {races.length === 0 && !loading && (
        <div className="text-center py-12 bg-[#101626] border border-emerald-500/20 rounded-xl shadow-lg shadow-emerald-500/10">
          <Mountain className="w-16 h-16 text-zinc-600 mx-auto mb-4" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {history.map((race, idx) => (
          ...
          >
            ...
            {race.stage && <p className="text-sm text-zinc-400 mb-2">{race.stage}</p>}
            <p className="text-xs text-zinc-500 mb-4">{race.location}</p>
            ...
            {race.top_riders && race.top_riders.length > 0 && (
              <div className="mt-4 pt-4 border-t">
                ...
                <div className="space-y-2">
                  {race.top_riders.slice(0, 2).map((rider: any, ridx: number) => (
                    <div key={ridx} className="flex items-center justify-between text-sm">
                      <span className="font-medium">{rider.name}</span>
                      <span className="text-zinc-400">DCI {rider.dci.dci_score}</span>
                    </div>
                  ))}
                </div>
              </div>
